...
def main(args):  # 接收 args 参数
    ...
    # Run model
    handler = reader_name(args)
    handler.LoadData(args)
    runner = runner_name(args, handler)  # 确保传递 args
    # logging.info('Test Before Training: ' + runner.print_res(data_dict['test']))
    if args.load > 0:
        model.load_model()
    if args.train > 0:
        runner.train(data_dict)  # 传递 data_dict
    ...
    #     save_rec_results(data_dict['dev'], runner, 100, args)  # 传递 args
    #     save_rec_results(data_dict['test'], runner, 100, args)  # 传递 args
    model.actions_after_train()
    logging.info(os.linesep + '-' * 45 + ' END: ' + utils.get_time() + ' ' + '-' * 45)


def save_rec_results(dataset, runner, topk, args):  # 添加 args 参数
...
if __name__ == '__main__':
    ...
    init_parser.add_argument('--model_name', type=str, default='AutoCF2', help='Choose a model to run.')
